...
### 2026-09-01 M9 交付：速度优化（单趟直方图 + 形状自适应并行，模型字节不变）

**决策**：M8 后从候选清单立项「速度优化」（此前对标记录 sooboost 慢于 LightGBM 3–9 倍）。约束：纯 Rust `#![forbid(unsafe_code)]` 不破、红线 3（任意线程数逐位一致）不破、精度门禁不回退。剖析结论：旧热路径每特征独立遍历节点行，`grad[r]`/`hess[r]` 被重复读取 F 倍（F=特征数）。

**动作**（并行结构经三轮迭代，同负载公平 A/B 定案）：
- `BinnedMatrix` 特征主序 → **行主序**（`bins[row*nf+feature]`），新增 `row_bins(r)`；直方图重构为全特征扁平 `HistSet`（单趟填充：每行只读一次 (g,h)，内层连续读该行全部 bin）。
- 并行结构迭代：每节点特征维并行（旧）→ 层内节点并行（california 提速但 breast_cancer 并行度塌缩）→ 两阶段扁平（california 最优但 breast_cancer **慢 79%**）→ **形状自适应双路**：行数/特征数 ≥ 1024 走行主序两阶段（单元 = (节点×行块) 填充 + (节点×特征) 扫描，块 4096 行）；否则走 (节点×特征) 直接路径（每单元独立填单特征直方图并扫描，累加顺序与 v0.2.0 **逐位一致**）。
- 确定性：选路只依赖数据形状（与线程数无关）；行块边界只依赖节点行数、块间按块下标定序归并；(gain, feature) 全序合并与顺序无关；叶子值/节点 (G,H) 合计始终行序逐点累加不参与分块归并。
- 测量方法学：桌面负载使跨轮绝对耗时不可比（同代码两轮测出 1.76s 与 0.68s），最终结论一律基于**新旧二进制同轮交替 A/B**。

**验证**：
- 定版 A/B（best-of-4 交替）：california 1.592→**1.335s（+16%）**、diabetes 0.588→0.615s（单独 6 轮复测中位数 0.723→**0.631s（+13%）**，首轮 -4% 为噪声）、breast_cancer 0.345→**0.227s（+34%）**。
- 三集指标与旧版**逐位一致**（0.8403341506124684 / 0.3876609594474578 / 0.9950396825396826）——零模型漂移。
- workspace 127 测试全绿、fmt/clippy `-D warnings` 干净、三道基准门禁全过。

**限制**：多分类与标量共享同一建树内核，自动受益；SIMD 内核需 unsafe 或额外依赖（`safe_arch` 等），显式留远期；直方图分块归并以 ulp 级扰动分裂增益的可能性仅存在于行主序路径（当前仅 california 形状触发，实测未漂移）。

**待手动验收**：无——CI 以 GitHub Actions 实跑为准。**M9 出口关闭（2026-09-01）**。下一里程碑未立项；候选：crates.io 随下一个用户可见功能再发版、WASM/C ABI、直方图减法（子节点 = 父 − 兄弟，再省一半填充）。

### 2026-09-01 M10 交付：直方图减法（行主序路填充减半，模型字节不变）

**决策**：M9 后继续热路径优化，落地经典直方图减法（LightGBM/XGBoost 同款）：每对兄弟只直接构建**较小子节点**（seed，平局取左）的直方图，较大子节点 = 父 − seed。直接路径（多特征小行数）完全不动——保住「累加顺序与 v0.2.0 逐位一致」的性质。

**动作**：
- `HistSet::subtract_in_place`：grad/hess/miss 单步浮点减法，count 用 saturating_sub（子行集 ⊆ 父行集，恒非负，防御性）。
- builder 行主序路改三阶段（各一次 rayon join）：阶段 A 填 seed（(节点×行块) 单元）→ 阶段 B 推导非 seed → 阶段 C 全体 (节点×特征) 扫描。bookkeeping 为每个分裂登记 (父下标, 兄弟下标, 是否 seed)。
- **关键实现点**：推导子节点**接管**父直方图缓冲（`prev_hists[p].take()`）而非克隆——首版 clone+subtract 实测把填充减半的收益几乎吃光（每推导节点 41KB memcpy）；因分裂节点的两个子节点中恰有一方推导，take 语义安全。
- 确定性（红线 3）：seed 选择只依赖行数比较、减法单步浮点、父缓冲值确定 → 任意线程数逐位一致。

**验证**：
- 测量方法：桌面多线程噪声过大（同二进制 1.6–2.7s 波动），改用 `RAYON_NUM_THREADS=1` 聚焦 A/B（红线 3 保证单线程=多线程结果，6 轮交替）：california 中位数 2.489→**1.817s（快 27%）**、best 2.224→1.699s（快 24%）。
- 三集指标逐位一致（0.8403341506124684 / 0.3876609594474578 / 0.9950396825396826）——行主序路也未发生 ulp 漂移。
- workspace 127 测试全绿、fmt/clippy `-D warnings` 干净、三道基准门禁全过。

**限制**：减法仅作用于行主序路（直接路数据集零收益也零风险）；推导链可能链式累积 ulp 误差（实测未出现）；每层多一次 join（推导阶段），在噪声下不可测但不劣。

**待手动验收**：无——CI 以 GitHub Actions 实跑为准。**M10 出口关闭（2026-09-01）**。下一里程碑未立项；候选：扫描内核优化（feature_total 预存/前缀向量化）、WASM/C ABI、crates.io 随下一个用户可见功能再发版。

### 2026-09-01 M11 交付：扫描内核优化（负结果，整体回退）

**决策**：M10 出口候选中选「feature_total 预存」——`HistSet` 增设 `tot_g/tot_h/tot_c`（每特征非缺失合计），fill 按行序增量维护、merge/subtract 同步维护，`feature_total` 从 O(bins) 求和变 O(1) 读取。目标：直方图减法后进一步压缩行主序路的扫描阶段。

**动作与发现**：
- 实现：`histogram.rs` tot 字段 + 三处同步维护；`builder.rs` 直接路径与行主序路均切换为 O(1) 合计读取。
- **发现一（精度红线）**：直接路径改为填充循环内按行序累计合计后，ulp 扰动翻转近平局分裂——breast_cancer AUC 0.9950→0.9931（跌出真实集前二），diabetes r2 −0.0007。直接路径即时回退为 bin 序事后求和（逐位同 v0.2.0）。
- **发现二（性能红线，决定性）**：同轮交替 A/B（`RAYON_NUM_THREADS=1`，M10/M11 二进制 5 轮交替，无一轮反超）：california M11 中位数 1.377s vs M10 1.314s——**净回归 −4.8%**。机理：tot 维护在 fill 最热内循环给每 (行×特征) 加 3 次累加，而省下的仅是每次 (节点×特征) 扫描的 O(bins) 求和；填充次数远大于扫描次数时，移动开销必亏。此前「+14%」为跨轮对比测量，被桌面负载噪声污染（第二次验证方法学红线）。

**决策（收口）**：**M11 整体回退**，源码 `git checkout` 回到 M10 状态。验证：三集指标全部逐位恢复 v0.2.0 基线（0.8403341506124684 / 0.3876609594474578 / 0.9950396825396826）、workspace 127 测试全绿、fmt/clippy 干净、三道基准门禁全过。最终形态：`feature_total` 保留 O(bins) 向量化求和（编译器自动向量化，实测开销可忽略）。
