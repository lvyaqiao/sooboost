...
> 状态：现行
> 类型：T2 台账（行级更新）｜写者：修者（发现者登记）｜读者：修 bug 前对照家族雷达
> 范围：BUG 分族谱系（七族，用于评估重构必要性）+ BUG 登记（每次只登记 BUG，不自动分族）
> 关联：doc/baseline/architecture.md（红线与决策，对治来源）；doc/research/GBDT三巨头bug分族与代码质量.md（统计证据）；doc/baseline/contracts.md（契约破坏登记处）
> 更新：2026-08-13（编号制废除，原 05 迁移）
...
### BUG-2026-08-19-01 分箱阈值 off-by-one：训练/预测在边界值 x==boundaries[k] 处方向不一致

- **发现**：M1-2 多分类集成测试（离散可分数据）预测准确率仅 0.93——连续数据几乎不命中边界值所以 M0 未暴露。
- **现象**：值恰好等于分裂阈值 `x == boundaries[k]` 时，训练分区（`bin(x) <= k`）把它分到右侧，树预测（`x <= threshold`）把它分到左侧；两类行为不一致。
- **根因**：`BinTable::bin_value` 用 `partition_point(b <= x)`（边界为不含上界），而树阈值用 `x <= threshold`（含上界）——分箱层与树内核的阈值语义契约未显式统一（映射族 4「语义契约散落各层」模式，本 bug 为单点考虑不周，不入新族）。
- **修复**：`bin_value` 改严格 `<`（`partition_point(b < x)`），边界为所属 bin 的**含上界**，与 `x <= threshold → 左子树` 严格一致（sooboost-core/src/binning/bintable.rs）。
- **测试守护**：多分类集成测试（可分离簇，准确率 >0.95）覆盖边界值命中路径；原 M0 全部测试回归通过。
- **教训**：分箱语义与分裂阈值语义必须显式对齐为同一契约，并加「值恰好等于边界」的针对性测试（易踩坑 4 的变体：离散/整数值边界）。
