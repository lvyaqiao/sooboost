...
| crate | 用途 | 采用原因 |
|---|---|---|
| `arrow` / `parquet` | 数据层底座 | Apache 官方实现，Polars/DataFusion 同款；零拷贝读 Parquet/Polars/DuckDB 是核心卖点；缺失值用原生 null 位图（对应 03 设计红线 2） |
| `rayon` | 并行 | 事实标准数据并行库；直方图/分裂搜索的并行由 rayon 承担（对应 03 D2） |
| `serde` + `serde_json` | 模型 JSON 格式 | 调试/跨语言可读；option schema 化（CatBoost 长处） |
| `bincode` / `postcard` | 模型紧凑二进制 | mmap 热加载；与 JSON 双格式并存（对应 03 D6） |
| `rand` + `rand_xoshiro` | 确定性 RNG | 分箱 sketch/采样带种子，结果逐位可复现（对应 03 红线 3） |
| `proptest` | 属性测试 | 分裂单调性、缺失值语义、序列化往返等性质守护 |
| `criterion` | 基准 | 基准门禁入 CI（LightGBM 无性能门禁的教训） |
| `cargo-fuzz` | 模糊测试 | 三巨头全无 fuzz 的差异化机会（对应 03 红线 6） |
| `clap` | CLI | 事实标准 |
...
| crate | 现状 | 处置 |
|---|---|---|
| `wide` | 稳定 SIMD 封装（f32x8/f64x4 等，跨平台） | **采用**，在树内核内封装一层 SIMD 抽象；显式 SIMD 优于 C++ 依赖自动向量化（想做的.md 痛点） |
...
| 模块 | 自建内容 | 对应架构 |
|---|---|---|
| `data` | arrow RecordBatch 之上的薄封装：缺失值语义唯一定义、类别/权重/group 元数据 | 03 D1 |
| `binning` | 确定性 quantile sketch、bin 表（随模型序列化） | 03 D4 |
| `tree` | 直方图构建 + 分裂搜索内核（泛型特化串行/rayon/SIMD）、推理优先 SoA 树表示 | 03 D2/D7 |
| `loss` | Loss trait + 内置损失集 + 注册表 | 03 D3 |
| `boosting` | GBDT 循环、采样、早停/CV/回调、TrainingContext | 03 D5 |
| `model` | 版本化模型格式、JSON/二进制序列化、热替换 | 03 D6 |
| 类别特征 | ordered target statistics | 03 D9 |
...
| `forust-ml`（jinlow/forust） | **5,434 行 Rust / 17 文件**，单人维护，近 90 天下载仅 2,367 | 半成品：功能子集（无原生类别/排序/生存），无重工业投入，正是"想做的.md"排除的科研玩具 |
...
| 选型 | 放弃原因 |
|---|---|
| 自造 DMatrix / 自造数据格式 | 破坏 arrow 零拷贝卖点；复制 XGBoost 数据层 bug 面（03 架构原因） |
| EFB / GOSS | 微软专利风险；纯 CPU 下收益有限（03 D9） |
| pyo3 Python 绑定 | 绑定层是三巨头最大维护负担源（调研族 7）；本项目定位 Rust 生态闭环 |
